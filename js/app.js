...
const BANNER_TL = [{
  ...
  overlay: { img: "assets/logos/logo-voiston-branco.png", position: "top-right" },
}];
...
// URLs dos iframes
const URL_PORTAL = "https://voiston-ai.github.io/PortalDigitalBrascrs/prototype4/index.html#";
...
const Views = {
  ...
  show(name) {
    ...
    if (name !== 'portal' && typeof fecharVideoPortal === 'function') fecharVideoPortal();
  }
};
  ...
  document.querySelectorAll('.logos').forEach(container => {
    ...
    slots.forEach(i => {
      ...
      if (LOGOS[i]) {
        ...
        img.src = LOGOS[i];
        img.alt = 'Logo';
        img.onerror = () => { box.removeChild(img); box.classList.add('placeholder'); };
        box.appendChild(img);
      }
...
function fecharVideoPortal() {
  ...
  if (v) { v.pause(); v.src = ''; }     // libera buffer
  m.remove();
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  initCarousel(document.getElementById('banner-tl'), BANNER_TL, INTERVALO_CARROSSEL_MS);
  initCarousel(document.getElementById('banner-tr'), BANNER_TR, INTERVALO_CARROSSEL_MS);
  initCarousel(document.getElementById('banner-bl'), BANNER_BL, INTERVALO_CARROSSEL_MS);
  initCarousel(document.getElementById('banner-br'), BANNER_BR, INTERVALO_CARROSSEL_MS);
